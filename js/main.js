...
let update = function()
{
	...
	game.multiplier = functions.getMultiplier().mul(1.01).mul(E(2).pow(game.infinities)).mul(E(1023).pow(game.eternities)).mul(E(Number.MAX_VALUE).pow(game.realities))
	...
	if (game.points.gte(Number.MAX_VALUE))
	{
		...
		game.gain = game.multiplier.pow(E(1).div(game.points.log10().div(E(2).log10()).sub(1023).div(game.softcapWeaken)))
	}
	else
	{
